## OpenAPI Auto-Generated [http-client](https://www.stackage.org/lts-10.0/package/http-client-0.5.7.1) Bindings to `Asana`

The library in `lib` provides auto-generated-from-OpenAPI [http-client](https://www.stackage.org/lts-10.0/package/http-client-0.5.7.1) bindings to the Asana API.
...
| baseModule                      | Set the base module namespace                                                                                                 |          | Asana                      |
| cabalPackage                    | Set the cabal package name, which consists of one or more alphanumeric words separated by hyphens                             |          | asana-api                    |
| cabalVersion                    | Set the cabal version number, consisting of a sequence of one or more integers separated by dots                              | 0.1.0.0  | 1.0.0.0                    |
| customTestInstanceModule        | test module used to provide typeclass instances for types not known by the generator                                          |          |         |
| configType                      | Set the name of the type used for configuration                                                                               |          | AsanaConfig                      |
...
| requestType                     | Set the name of the type used to generate requests                                                                            |          | AsanaRequest                     |
...
| MODULE              | NOTES                                               |
| ------------------- | --------------------------------------------------- |
| Asana.Client    | use the "dispatch" functions to send requests       |
| Asana.Core      | core funcions, config and request types             |
| Asana.API       | construct api requests                              |
| Asana.Model     | describes api models                                |
| Asana.MimeTypes | encoding/decoding MIME types (content-types/accept) |
| Asana.ModelLens | lenses for model fields                             |
| Asana.Logging   | logging functions and utils                         |
