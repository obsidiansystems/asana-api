...
-- | display the config
instance P.Show AsanaConfig where
  show c =
    T.printf
...
-- | constructs a default AsanaConfig
--
-- configHost:
--
...
-- @"asana-api/1.0.0.0"@
--
newConfig :: IO AsanaConfig
newConfig = do
    logCxt <- initLogContext
    return $ AsanaConfig
        { configHost = "https://app.asana.com/api/1.0"
        , configUserAgent = "asana-api/1.0.0.0"
...
addAuthMethod :: AuthMethod auth => AsanaConfig -> auth -> AsanaConfig
addAuthMethod config@AsanaConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: AsanaConfig -> IO AsanaConfig
...
withStderrLogging :: AsanaConfig -> IO AsanaConfig
...
withNoLogging :: AsanaConfig -> AsanaConfig
...
data AsanaRequest req contentType res accept = AsanaRequest
  { rMethod  :: NH.Method   -- ^ Method of AsanaRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of AsanaRequest
  , rParams   :: Params -- ^ params of AsanaRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (AsanaRequest req contentType res accept) NH.Method
rMethodL f AsanaRequest{..} = (\rMethod -> AsanaRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (AsanaRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f AsanaRequest{..} = (\rUrlPath -> AsanaRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (AsanaRequest req contentType res accept) Params
rParamsL f AsanaRequest{..} = (\rParams -> AsanaRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (AsanaRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f AsanaRequest{..} = (\rAuthTypes -> AsanaRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => AsanaRequest req contentType res accept -> param -> AsanaRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: AsanaRequest req contentType res accept -> param -> AsanaRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: AsanaRequest req contentType res accept -> param -> AsanaRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> AsanaRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = AsanaRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: AsanaRequest req contentType res accept -> [NH.Header] -> AsanaRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: AsanaRequest req contentType res accept -> [NH.HeaderName] -> AsanaRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => AsanaRequest req contentType res accept -> AsanaRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => AsanaRequest req contentType res accept -> AsanaRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: AsanaRequest req contentType res accept -> [NH.QueryItem] -> AsanaRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: AsanaRequest req contentType res accept -> WH.Form -> AsanaRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: AsanaRequest req contentType res accept -> NH.Part -> AsanaRequest req contentType res accept
...
_setBodyBS :: AsanaRequest req contentType res accept -> B.ByteString -> AsanaRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: AsanaRequest req contentType res accept -> BL.ByteString -> AsanaRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => AsanaRequest req contentType res accept -> P.Proxy authMethod -> AsanaRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: AsanaConfig
    -> a
    -> AsanaRequest req contentType res accept
    -> IO (AsanaRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> AsanaConfig
  -> IO (AsanaRequest req contentType res accept)
_applyAuthMethods req config@(AsanaConfig {configAuthMethods = as}) =
  foldlM go req as
  where
